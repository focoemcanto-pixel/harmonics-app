...
function EquipeContent() {
  ...
  const carregarEquipe = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/workspace/team', { method: 'GET', cache: 'no-store' });
  ...
  async function removerMembro(member) {
    ...
    try {
      const response = await fetch(`/api/workspace/team/${member.id}`, { method: 'DELETE' });
      const payload = await response.json().catch(() => null);
          ...
          <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_12px_32px_rgba(15,23,42,0.05)]">
            ...
            <form onSubmit={convidarMembro} className="mt-5 space-y-4">
              ...
              <button
                type="submit"
                disabled={saving}
                className="w-full rounded-[18px] bg-violet-600 px-5 py-4 text-sm font-black text-white shadow-lg shadow-violet-600/20 transition hover:bg-violet-700 disabled:cursor-not-allowed disabled:opacity-60"
              >
          ...
          <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-[0_12px_32px_rgba(15,23,42,0.05)]">
            ...
            <div className="mt-5">
              {loading ? (
                ...
                <div className="rounded-[24px] border border-dashed border-slate-200 bg-slate-50 p-8 text-center text-sm font-semibold text-slate-500">
                    ...
                    return (
                      <div key={member.id} className="rounded-[24px] border border-slate-200 bg-slate-50 p-4">
                        ...
                        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-[1fr_auto]">
                          ...
                          <button
                            type="button"
                            onClick={() => removerMembro(member)}
                            disabled={isBusy || isCurrent}
                            className="rounded-2xl border border-red-200 bg-white px-4 py-3 text-sm font-black text-red-600 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"
                          >
