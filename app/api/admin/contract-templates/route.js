...
const SCHEMA_MIGRATION_REQUIRED = 'A tabela contract_templates ainda não possui workspace_id. Aplique a migration de workspace scoping antes de criar ou listar templates neste workspace.';
...
async function listTemplatesScoped(supabaseAdmin, workspaceId) {
...
export async function POST(request) {
  ...
  try {
    ...
    if (payload.is_default) {
      ...
      if (clearError) {
        if (isMissingWorkspaceColumnError(clearError)) {
          return NextResponse.json({ ok: false, migrationRequired: true, error: SCHEMA_MIGRATION_REQUIRED }, { status: 428 });
        }
        throw clearError;
      }
    }
