...
async function upsertEventFromSignature({
  ...
  const payload = {
    ...
    event_date: form.event_date || precontract?.event_date || null,
...
export default function ContratoPublicoPage() {
      ...
      try {
        ...
        if (preData?.id) {
          ...
          setForm({
            ...
            event_date: saved.event_date || preData.event_date || '',
  ...
  function validateMainFields() {
    const required = [
      { ok: form.full_name.trim(), label: 'nome completo' },
      { ok: form.marital_status.trim(), label: 'estado civil' },
      { ok: form.profession.trim(), label: 'profissão' },
      { ok: form.cpf.trim(), label: 'CPF' },
      { ok: form.rg.trim(), label: 'RG' },
      { ok: form.whatsapp.trim(), label: 'WhatsApp' },
      { ok: form.address_street.trim(), label: 'rua/endereço' },
      { ok: form.address_neighborhood.trim(), label: 'bairro' },
      { ok: form.address_cep.trim(), label: 'CEP' },
      { ok: form.address_city.trim(), label: 'cidade' },
      { ok: form.address_state.trim(), label: 'estado' },
      { ok: form.event_date.trim(), label: 'data do evento' },
      { ok: form.event_time.trim(), label: 'horário do evento' },
      { ok: form.event_location_name.trim(), label: 'local do evento' },
      { ok: form.event_location_address.trim(), label: 'endereço do evento' },
    ];

    const missing = required.find((item) => !item.ok);
    if (missing) {
  ...
  async function upsertContract(statusOverride = 'client_filling') {
    ...
    const clientForm = {
      ...
      signer_cpf: form.signer_cpf.trim() || null,
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
          <div className="space-y-6 xl:col-span-2">
            ...
            <Card>
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <Input
                  ...
                  value={form.whatsapp}
                  onChange={(e) =>
                    handleChange('whatsapp', maskPhone(e.target.value))
                  }
                />
              </div>
            </Card>
            ...
            <Card>
              <SectionTitle subtitle="Esses dados serão usados no contrato. Depois vamos plugar autocomplete do Google.">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleChange('address_cep', maskCep(e.target.value))
                  }
                />
            ...
            <Card>
              <SectionTitle subtitle="Confirme os dados finais do evento com atenção.">
                Dados do evento
              </SectionTitle>
            ...
            <Card title="Assinatura eletrônica">
              <div className="space-y-4">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleChange('signer_cpf', maskCpf(e.target.value))
                  }
                />
