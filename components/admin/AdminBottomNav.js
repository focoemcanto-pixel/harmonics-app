...
function NavItem({ item, active, onClick }) {
  ...
  if (item.key === 'mais') {
    return (
      <button
        type="button"
        onClick={onClick}
        className="touch-manipulation pointer-events-auto flex min-h-[56px] flex-col items-center justify-center rounded-2xl px-2 py-2 text-center transition"
        style={activeStyle}
      >
        {content}
      </button>
  ...
  return (
    <Link
      href={item.href}
      className="touch-manipulation flex min-h-[56px] flex-col items-center justify-center rounded-2xl px-2 py-2 text-center transition"
      style={activeStyle}
    >
      {content}
    </Link>
