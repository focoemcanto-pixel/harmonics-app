...
export default function MembroPlayerModal({ open, eventTitle, playlist = [], currentIndex = 0, isPlaying, onClose, onSelectTrack, onPrev, onNext, onTogglePlay }) {
  ...
  return <div className="fixed inset-0 z-[150] bg-black/70 backdrop-blur-[4px]"><div className="flex min-h-[100dvh] items-end justify-center pt-[env(safe-area-inset-top,0px)] lg:min-h-screen lg:items-center lg:p-6"><div className="flex h-[min(92dvh,calc(100dvh-env(safe-area-inset-top,0px)-env(safe-area-inset-bottom,0px)))] w-full min-h-0 flex-col overflow-hidden rounded-t-[30px] border border-white/10 bg-[#0b1020] text-white shadow-[0_24px_80px_rgba(0,0,0,0.45)] lg:h-auto lg:max-h-[92vh] lg:max-w-5xl lg:rounded-[30px]">
    ...
    <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-5 pb-[calc(env(safe-area-inset-bottom,0px)+20px)] pt-5 [-webkit-overflow-scrolling:touch] lg:hidden"><div className="flex min-w-0 flex-col gap-5"><PlayerStatusBadge isPlaying={isPlaying} /><NowPlayingCard currentTrack={currentTrack} /><YoutubePlaybackFrame title={currentTrack?.title} isPlaying={isPlaying} thumbnailUrl={thumbnailUrl} onTogglePlay={onTogglePlay} /><PlayerControls isPlaying={isPlaying} currentTrack={currentTrack} onPrev={onPrev} onNext={onNext} onTogglePlay={onTogglePlay} /><TrackList playlist={playlist} currentIndex={currentIndex} onSelectTrack={onSelectTrack} /></div></div>
    <div className="hidden flex-1 min-h-0 min-w-0 overflow-hidden lg:grid lg:grid-cols-[1.05fr_0.95fr]"><div className="min-h-0 min-w-0 overflow-y-auto border-r border-white/10 p-6"><div className="flex min-h-0 flex-col gap-5"><PlayerStatusBadge isPlaying={isPlaying} /><NowPlayingCard currentTrack={currentTrack} /><YoutubePlaybackFrame title={currentTrack?.title} isPlaying={isPlaying} thumbnailUrl={thumbnailUrl} onTogglePlay={onTogglePlay} /><PlayerControls desktop isPlaying={isPlaying} currentTrack={currentTrack} onPrev={onPrev} onNext={onNext} onTogglePlay={onTogglePlay} /></div></div><div className="min-h-0 min-w-0 overflow-y-auto p-6"><TrackList desktop playlist={playlist} currentIndex={currentIndex} onSelectTrack={onSelectTrack} /></div></div>
  </div></div></div>;
}
