    ...
    <article className="rounded-[28px] border border-white/10 bg-white/5 p-5 text-white shadow-[0_14px_34px_rgba(15,23,42,0.16)] backdrop-blur">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="min-w-0">
          ...
          <h3 className="mt-2 break-words text-[24px] font-black tracking-[-0.04em]">
            {item.clientName}
          </h3>

          <div className="mt-3 flex flex-wrap items-center gap-2">
            <span className={`inline-flex max-w-full break-words rounded-full border px-3 py-1 text-[11px] font-black uppercase tracking-[0.08em] ${tone.badge}`}>
              {tone.label}
            </span>

            {item.suggestedRoleName ? (
              <span className="inline-flex max-w-full break-words rounded-full border border-violet-300/15 bg-violet-400/10 px-3 py-1 text-[11px] font-black uppercase tracking-[0.08em] text-violet-100">
                {item.suggestedRoleName}
              </span>
            ) : null}
          </div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2">
        <div className="rounded-[18px] border border-white/10 bg-black/10 px-4 py-4">
          ...
          <div className="mt-2 break-words text-[15px] font-semibold">
      ...
      {item.instruments ? (
        <div className="mt-4 rounded-[18px] border border-white/10 bg-black/10 px-4 py-4">
          ...
          <div className="mt-2 break-words text-[14px] leading-7 text-white/85">
            {item.instruments}
          </div>
        </div>
      ) : null}
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <button
          ...
          disabled={loadingKey === keyAccept || isBusy}
          aria-busy={loadingKey === keyAccept ? 'true' : undefined}
          className="min-h-12 touch-manipulation rounded-[18px] bg-gradient-to-r from-emerald-500 to-green-500 px-5 py-4 text-[15px] font-black text-white shadow-[0_14px_28px_rgba(16,185,129,0.25)] transition active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60 disabled:active:scale-100"
        >
        ...
        <button
          ...
          disabled={loadingKey === keyDecline || isBusy}
          aria-busy={loadingKey === keyDecline ? 'true' : undefined}
          className="min-h-12 touch-manipulation rounded-[18px] bg-gradient-to-r from-rose-500 to-red-500 px-5 py-4 text-[15px] font-black text-white shadow-[0_14px_28px_rgba(239,68,68,0.22)] transition active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60 disabled:active:scale-100"
        >
