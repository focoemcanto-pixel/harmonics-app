    ...
    <div className="fixed inset-x-0 bottom-[calc(env(safe-area-inset-bottom,0px)+82px)] z-[140] px-3 pb-2 md:bottom-4 md:px-6">
      <div className="mx-auto max-w-4xl overflow-hidden rounded-[24px] border border-white/10 bg-[linear-gradient(135deg,rgba(10,14,30,0.98),rgba(37,23,68,0.98))] text-white shadow-[0_18px_60px_rgba(0,0,0,0.48)] backdrop-blur-xl">
        ...
        <div className="flex items-center gap-3 px-3 py-3 sm:px-4">
          ...
          >
            {thumbnailUrl ? (
              <img src={thumbnailUrl} alt={currentTrack?.title || 'Thumbnail'} className="h-full w-full object-cover" />
            ) : (
              <div className="flex h-full w-full items-center justify-center text-[18px] text-white">♫</div>
            )}
          ...
          <button type="button" onClick={onExpand} className="min-w-0 flex-1 touch-manipulation text-left active:scale-[0.99]">
            ...
            <div className="mt-0.5 truncate text-[10px] font-black uppercase tracking-[0.07em] text-fuchsia-200/65">
              {currentTrack?.subtitle || eventTitle || 'Repertório'}
            </div>
          </button>

          <div className="flex shrink-0 items-center gap-1.5">
            <button type="button" onClick={(e) => { e.stopPropagation(); onPrev?.(); }} className="hidden h-10 w-10 items-center justify-center rounded-[13px] border border-white/10 bg-white/8 text-[14px] font-black active:scale-95 sm:flex" aria-label="Faixa anterior">‹</button>
            ...
            <button type="button" onClick={(e) => { e.stopPropagation(); onNext?.(); }} className="flex h-10 w-10 items-center justify-center rounded-[13px] border border-white/10 bg-white/8 text-[14px] font-black active:scale-95" aria-label="Próxima faixa">›</button>
            <button type="button" onClick={(e) => { e.stopPropagation(); setCollapsed(true); }} className="flex h-10 w-9 items-center justify-center rounded-[13px] border border-white/10 bg-white/5 text-[13px] font-black text-white/70 active:scale-95" aria-label="Recolher mini player">→</button>
