...
export async function requireAdminFromRequest({ supabase, request, logPrefix = '[ADMIN_GUARD]' }) {
  ...
  if (!token) {
    console.warn(`${logPrefix}[DELETE][USER]`, { message: 'missing_bearer_token' });
  ...
  if (userError || !userData?.user?.id) {
    console.warn(`${logPrefix}[DELETE][USER]`, { message: userError?.message || 'missing_user' });
    return { ok: false, status: 401, error: 'Sessão expirada. Faça login novamente.' };
  }

  const userId = userData.user.id;
  console.info(`${logPrefix}[DELETE][USER]`, { userId });
  ...
  if (profileError) {
    ...
    if (!missingIsAdminColumn) {
      console.error(`${logPrefix}[DELETE][PROFILE]`, {
    ...
    console.warn(`${logPrefix}[DELETE][PROFILE]`, {
