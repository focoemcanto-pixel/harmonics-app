...
export async function buildAutomationContext(eventType, entityId, recipientData) {
  ...
  if (eventType === 'contract_signed_admin') {
    const { precontract, contract } = recipientData.contextData || {};
    setContextValue('cliente_nome', precontract?.client_name || '');
    setContextValue('evento_data', formatDateBR(precontract?.event_date));
    setContextValue('evento_horario', formatTime(precontract?.event_time));
    setContextValue('link_painel_cliente', precontract?.public_token ? `${baseUrl}/cliente/${precontract.public_token}` : '');
    setContextValue('link_contrato', precontract?.public_token ? `${baseUrl}/contrato/${precontract.public_token}` : '');
    setContextValue('pdf_url', contract?.pdf_url || '');
    setContextValue('link_pdf', contract?.pdf_url || '');
    return context;
  }
  ...
  if (CLIENT_EVENT_TYPES.includes(eventType)) {
    ...
    const clientName = precontract?.client_name || event?.client_name || '';
    const eventDate = formatDateBR(event?.event_date);
    const eventTime = formatTime(event?.event_time);

    setContextValue('cliente_nome', clientName);
    setContextValue('client_name', clientName);
    setContextValue('nome_empresa', 'Harmonics');
    setContextValue('evento_nome', event?.client_name || '');
    setContextValue('evento_data', eventDate);
    setContextValue('evento_horario', eventTime);
    setContextValue('event_time', eventTime);
    setContextValue('evento_local', event?.location_name || '');
    setContextValue('event_location', event?.location_name || '');
    setContextValue('event_name', event?.client_name || '');
    setContextValue('event_date', eventDate);
    ...
    if (eventType === 'payment_pending_2_days_client') {
      const paymentLink = precontract?.public_token
        ? `${baseUrl}/cliente/${precontract.public_token}?tab=pagamentos`
        : painelClienteLink;
      setContextValue('payment_link', paymentLink);
      setContextValue('link_pagamento', paymentLink);
    }
