  ...
  try {
    switch (eventType) {
      ...
      case 'event_day_confirmation_client':
        return await getEventDayConfirmationCandidates(supabaseAdmin);
 ...
 * - evento de manhã (<14:00): envia no dia anterior às 17:00
 * - evento >=14:00: envia ~5h antes
 * - nunca fora de 09:00-18:00 (ajuste para horário comercial válido mais próximo)
 * - candidato elegível se "agora SP" estiver até 90 minutos após o horário calculado
 */
async function getEventDayConfirmationCandidates(supabase) {
  ...
  for (const event of events) {
    ...
    if (diffMs >= 0 && diffMs <= 90 * 60 * 1000) {
      console.log('[AUTOMATION][EVENT_DAY_MESSAGE]', {
        eventId: event.id,
