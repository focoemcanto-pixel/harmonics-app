...
function normalizeWorkspaceSettings(data) {
  ...
  const workspaceSettingsId = explicitWorkspaceSettingsId || String(data?.workspace_settings_id || data?.id || '').trim() || null;
  ...
  // Portanto, os campos normalizados precisam prevalecer sobre qualquer
  // id legado/nulo vindo no payload original.
  normalized.id = resolvedWorkspaceId;
  normalized.workspaceId = resolvedWorkspaceId;
  normalized.workspace_id = resolvedWorkspaceId;
  normalized.workspaceSettingsId = workspaceSettingsId;
  normalized.workspace_settings_id = workspaceSettingsId;
  normalized.migrationMode = migrationMode;
  normalized.ai_enabled = Boolean(data?.ai_enabled);
  normalized.ai = {
