...
import { getCurrentAutomationWorkspaceSettings, getDefaultWorkspaceSettings } from '@/lib/automation/get-workspace';
...
async function resolveAutomationWorkspace({ supabase, request } = {}) {
  const client = supabase || getSupabaseAdmin();
  const workspace = request
    ? await getCurrentAutomationWorkspaceSettings({ supabase: client, request })
    : await getDefaultWorkspaceSettings(client);

  return {
    workspace,
    workspaceId: asUuidOrNull(workspace?.id),
    migrationMode: !asUuidOrNull(workspace?.id),
  };
}
...
async function ensureTemplate(supabase, workspaceId) {
  ...
  const { data: existing, error: selectError } = await scopeWorkspace(baseSelect, workspaceId).maybeSingle();
...
export async function getPaymentDue2DaysConfig(options = {}) {
  const supabase = options.supabase || getSupabaseAdmin();
  const { workspaceId, migrationMode } = await resolveAutomationWorkspace({ supabase, request: options.request });

  const template = await ensureTemplate(supabase, workspaceId);
  const rule = await ensureRule(supabase, workspaceId, template?.id || null);

  const isEnabled = Boolean(workspaceId) && rule?.is_active !== false;
...
export async function updatePaymentDue2DaysConfig({ isEnabled, message, request = null } = {}) {
  const supabase = getSupabaseAdmin();
  const config = await getPaymentDue2DaysConfig({ supabase, request });

  if (!config.workspaceId) {
    throw new Error('Workspace de automação não resolvido. Configure workspace_settings antes de alterar o lembrete.');
  }
  ...
  return getPaymentDue2DaysConfig({ supabase, request });
}
...
export async function runPaymentDue2DaysReminder({ forcedTargetDate = null } = {}) {
  const supabase = getSupabaseAdmin();
  const { workspaceId } = await resolveAutomationWorkspace({ supabase });
  ...
  const config = await getPaymentDue2DaysConfig({ supabase });
