 ...
 * @param {string} tipo - 'musician' | 'staff' | 'vendor' | 'client' | 'all'
 * @returns {array} Contatos filtrados
 */
export function filterByType(contatos, tipo) {
  if (!tipo || tipo === 'all') return contatos;
  return contatos.filter((c) => c.contact_type === tipo);
}
