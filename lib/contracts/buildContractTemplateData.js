...
function buildExtrasText(precontract, event) {
  ...
  const receptionFormation = safe(precontract?.reception_formation) || safe(event?.reception_formation);
  const receptionInstruments = safe(precontract?.reception_instruments) || safe(event?.reception_instruments);
  const hasSound =
    Boolean(precontract?.has_sound ?? event?.has_sound ?? false);
  const hasTransport =
    Boolean(precontract?.has_transport ?? event?.has_transport ?? false);
  const hasAntesala = Boolean(event?.has_antesala ?? event?.antesala_enabled ?? false);
  const antesalaMinutes = Number(event?.antesala_duration_minutes ?? 0) || 0;

  if (receptionHours > 0) {
    const receptionText = receptionFormation && receptionInstruments
      ? `Receptivo de ${receptionHours}h — ${receptionFormation} (${receptionInstruments})`
      : `Receptivo de ${receptionHours}h`;
    extras.push(receptionText);
  }

  if (hasAntesala) {
    if (antesalaMinutes > 0) {
      extras.push(`Antesala incluída — ${event?.formation || precontract?.formation || 'Formação'} — ${antesalaMinutes} min`);
    } else {
      extras.push(`Antesala incluída — ${event?.formation || precontract?.formation || 'Formação'}`);
    }
  }
...
export function buildContractTemplateData({
  ...
  const receptionFormation =
    safe(event?.reception_formation) || safe(precontract?.reception_formation);
  const receptionInstruments =
    safe(event?.reception_instruments) || safe(precontract?.reception_instruments);
  const receptionHours =
    Number(precontract?.reception_hours ?? event?.reception_hours ?? 0) || 0;
  const receptionText = receptionHours > 0
    ? (receptionFormation && receptionInstruments
      ? `${receptionFormation} (${receptionInstruments})`
      : receptionFormation || receptionInstruments || '')
    : '';
  ...
  const receptionHours = Number(precontract?.reception_hours ?? event?.reception_hours ?? 0) || 0;
  const receptionFormation = safe(precontract?.reception_formation) || safe(event?.reception_formation);
  const receptionInstruments = safe(precontract?.reception_instruments) || safe(event?.reception_instruments);
  const receptionText = receptionHours > 0
    ? (receptionFormation && receptionInstruments
...
const balanceDate =
  ...
  return {
    ...
    RECEPTIVO_FORMACAO: receptionFormation,
    RECEPTIVO_INSTRUMENTOS: receptionInstruments,
    RECEPTIVO_TEXTO: receptionText,
    ...
    RECEPTIVO: receptionHours > 0,
    RECEPTIVO_HORAS: receptionHours ? `${receptionHours}h` : '',
    RECEPTIVO_FORMACAO: receptionFormation,
    RECEPTIVO_INSTRUMENTOS: receptionInstruments,
    RECEPTIVO_TEXTO: receptionText,
    EXTRAS: hasExtras,
    ...
    formation,
    instruments,
    reception_formation: receptionFormation,
    reception_instruments: receptionInstruments,
    reception_text: receptionText,
    event_date: formatDateBR(eventDate),
    event_time: formatTimeBR(eventTime),
    event_location: formatEventLocation(eventLocationName, eventLocationAddress),
    total_amount: formatMoneyBR(agreedAmount),
    total_amount_extenso: numberToPortuguese(agreedAmount),
    deposit_due_date: signalDate ? formatDateBR(signalDate) : '',
    balance_due_date: balanceDate ? formatDateBR(balanceDate) : '',
    card_due_date: cardDate ? formatDateBR(cardDate) : '',
    receptivo: receptionHours > 0,
    reception: receptionHours > 0,
    reception_hours: receptionHours,
    reception_formation: receptionFormation,
    reception_instruments: receptionInstruments,
    reception_text: receptionText,
    extras: hasExtras,
    card: hasCard && !!cardDate,
    extras_text: hasExtras ? extrasText : '',
    client_signature: signatureName,
    accepted_name: signatureName,
    accepted_cpf: signatureCpf,
    accepted_datetime: signedAt,
    accepted_ip: safe(contract?.signature_ip),
    accepted_origin: 'Sistema Harmonics',
    contract_token: token,
    document_hash: buildDocumentHash({
      contractId,
      token,
      signedAt,
    }),
    signature_stamp: buildSignatureStamp({
      signatureName,
      signatureCpf,
      signedAt,
    }),
