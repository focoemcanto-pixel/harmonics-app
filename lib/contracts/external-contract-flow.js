...
export async function ensureClientPanelForExternalContract({ supabase, eventId, contactId, existingContract, pdfUrl, rawPayload, replace }) {
...
export async function saveExternalContractForEvent({ supabase, eventId, file, replace = false, contactId = null, rawPayload = {} }) {
  ...
  const { data: existingContract, error: contractError } = await supabase.from('contracts').select('*').eq('event_id', eventId).order('created_at', { ascending: false }).limit(1).maybeSingle();
  ...
  const result = await ensureClientPanelForExternalContract({ supabase, eventId, contactId, existingContract, pdfUrl, rawPayload, replace });
