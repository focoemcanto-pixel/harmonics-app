...
import { resolveContractHtmlSource } from './resolveContractHtmlSource';
import { resolveEventTypeDefaultTemplateId } from './precontract-template';
...
async function hydrateInternalTemplate({ supabase, context }) {
  let templateId = asString(context?.precontract?.contract_template_id);
  if (!templateId && context?.precontract?.event_type_id) {
    templateId = await resolveEventTypeDefaultTemplateId({
      supabase,
      workspaceId: context?.precontract?.workspace_id,
      eventTypeId: context?.precontract?.event_type_id,
    });

    if (templateId && context?.precontract?.id) {
      await supabase
        .from('precontracts')
        .update({
  ...
  if (!templateId) return { context, template: null, source: 'precontract' };
  ...
  if (error) throw new Error(`Erro ao buscar template de contrato: ${error.message}`);

  if (!template?.id) {
    return { context, template: null, source: 'missing_template' };
  }

  const resolvedTemplate = resolveContractHtmlSource(template);
  if (!asString(resolvedTemplate.html)) {
    return { context, template, source: 'template_without_html' };
  }
  ...
  return {
    context: {
      ...
      precontract: {
        ...context.precontract,
        custom_contract_enabled: true,
        custom_contract_rich_html: resolvedTemplate.html,
        custom_contract_content: resolvedTemplate.html,
        contract_template_text: resolvedTemplate.html,
      },
    },
    template,
    source: `contract_templates.${resolvedTemplate.sourceField || 'content'}`,
  };
}
...
export async function generateContractDocument({ supabase, contractId, precontractId, previewOnly = false }) {
  ...
  const internalTemplate = isInternal
    ? await hydrateInternalTemplate({ supabase, context })
    : { context, template: null, source: 'not_internal' };
  const effectiveContext = internalTemplate.context;
  const templateData = buildContractTemplateData(effectiveContext);

  console.info('[CONTRACT_ENGINE_RESOLVED]', {
    engine: engineContext.engine,
    source: engineContext.source,
    workspaceId: engineContext.workspace?.id || effectiveContext?.precontract?.workspace_id || null,
    workspaceSlug: engineContext.workspace?.slug || engineContext.workspace?.key || null,
    precontractId: effectiveContext.precontract?.id || null,
    contractId: effectiveContext.contract?.id || null,
    isInternal,
    internalTemplateSource: internalTemplate.source,
    internalTemplateId: internalTemplate.template?.id || null,
  });

  if (isInternal) {
    const internal = generateInternalContract(effectiveContext, templateData);

    if (!asString(internal.html)) {
      return {
        ok: false,
        status: 422,
        mode: 'internal',
        error: internal.error || 'MISSING_INTERNAL_CONTRACT_HTML',
        message: internal.message || 'Contrato interno sem template/HTML real para geração.',
        ids: {
          contractId: effectiveContext.contract?.id || null,
          precontractId: effectiveContext.precontract?.id || null,
          contactId: effectiveContext.contact?.id || null,
          eventId: effectiveContext.event?.id || null,
        },
        templateData,
        context: effectiveContext,
        engine: engineContext.engine,
        templateSource: internalTemplate.source,
        templateId: internalTemplate.template?.id || null,
      };
    }
  ...
  if (previewOnly) {
    return {
      ...
      templateData: buildContractTemplateData(context),
      context,
      engine: engineContext.engine,
    };
  }
  ...
  let generated = await callContractService({
    ...
    payload: { templateId, rootFolderId, templateData: buildContractTemplateData(context), contractName, eventDate, placeholderStyle: 'double_curly' },
  });
