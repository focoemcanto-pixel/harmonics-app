...
import { normalizeTime } from '../time/normalize-time';
import { ensureInternalPrecontractTemplate } from './precontract-template';

function generateToken() {
  const generatedToken = `${Date.now().toString(36)}${Math.random().toString(36).slice(2, 10)}`;
  console.info('[CONTRACT_TOKEN_FLOW] token gerado', {
...
export async function generatePrecontractFromEvent(event) {
  ...
  if (existing) {
    const existingToken = String(existing.public_token || '').trim();
    const existingLink = String(existing.generated_link || '').trim();

    if (existingToken && existingLink) {
      return existing;
    }

    const token = existingToken || generateToken();
    const link = existingLink || buildContractLink(token);
    ...
    console.info('[CONTRACT_TOKEN_FLOW] token salvo (update precontracts)', {
      precontractId: updated?.id || existing.id,
      public_token: updated?.public_token || token,
      generated_link: updated?.generated_link || link,
    });
  ...
  const payload = {
    ...
    event_time: normalizeTime(event.event_time) || null,
  ...
  const safePayload = await ensureInternalPrecontractTemplate({
    supabase,
    workspaceId: event.workspace_id,
    payload: {
      ...payload,
      event_type_id: event.event_type_id || null,
      contract_template_id: event.contract_template_id || null,
    },
  });
  ...
  console.info('[CONTRACT_TOKEN_FLOW] token salvo (insert precontracts)', {
    precontractId: data?.id || null,
