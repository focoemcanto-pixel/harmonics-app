...
async function getGoogleAuth() {
  ...
  const {
    credentials: credentialsFromSupabase,
    source,
    validation,
  } = await loadGoogleCredentialsFromSupabase();

  const credentials = credentialsFromSupabase;
  const credentialsSource = source;
  const credentialsValidation = validation;

  console.info('[googleContractGenerator] OAuth diagnostics:', {
    hasClientId: Boolean(clientId),
    hasClientSecret: Boolean(clientSecret),
    hasRedirectUri: Boolean(redirectUri),
    credentialsSource,
    credentialsValidation: credentialsValidation?.reason,
    credentialsType: typeof credentials,
    credentialsIsArray: Array.isArray(credentials),
    refreshTokenResolved: maskToken(credentials?.refresh_token),
    accessTokenResolved: maskToken(credentials?.access_token),
  });
  ...
  if (!credentialsValidation?.valid) {
    throw new Error(
      `Credenciais OAuth Google inválidas (${credentialsValidation?.reason}) via ${credentialsSource}: ${credentialsValidation?.message}`
    );
  }

  if (!credentials || typeof credentials !== 'object' || Array.isArray(credentials)) {
    throw new Error('Credenciais OAuth carregadas em formato inválido (esperado objeto).');
  }

  const normalizedCredentialsValidation = validateGoogleCredentials(credentials);
  if (!normalizedCredentialsValidation.valid) {
    throw new Error(
      `Credenciais OAuth Google inválidas após normalização (${normalizedCredentialsValidation.reason}): ${normalizedCredentialsValidation.message}`
    );
  }

  const normalizedCredentials = normalizedCredentialsValidation.credentials;
  ...
  try {
    console.info('[googleContractGenerator] antes do setCredentials:', {
      oauth2CredentialsType: typeof oauth2Client.credentials,
      oauth2CredentialsIsArray: Array.isArray(oauth2Client.credentials),
      normalizedCredentialsType: typeof normalizedCredentials,
      normalizedCredentialsIsArray: Array.isArray(normalizedCredentials),
      hasRefreshToken: Boolean(normalizedCredentials?.refresh_token),
      hasAccessToken: Boolean(normalizedCredentials?.access_token),
      refreshTokenMasked: maskToken(normalizedCredentials?.refresh_token),
      accessTokenMasked: maskToken(normalizedCredentials?.access_token),
    });

    if (!oauth2Client.credentials || typeof oauth2Client.credentials !== 'object') {
      console.warn('[googleContractGenerator] credentials inválido antes de setCredentials.', {
        credentialsType: typeof oauth2Client.credentials,
      });
      oauth2Client.credentials = {};
    }

    oauth2Client.setCredentials({
      refresh_token: normalizedCredentials.refresh_token,
      access_token: normalizedCredentials.access_token,
      token_type: normalizedCredentials.token_type || 'Bearer',
      expiry_date: normalizedCredentials.expiry_date,
    });

    console.info('[googleContractGenerator] depois do setCredentials:', {
      oauth2CredentialsType: typeof oauth2Client.credentials,
      oauth2CredentialsIsArray: Array.isArray(oauth2Client.credentials),
      hasRefreshToken: Boolean(oauth2Client?.credentials?.refresh_token),
      hasAccessToken: Boolean(oauth2Client?.credentials?.access_token),
  ...
  function walkContent(content) {
    for (const item of content || []) {
      if (item?.paragraph?.elements) {
        walkElements(item.paragraph.elements);
      }
      if (item?.table) {
        walkTable(item.table);
      }
...
export async function generateGoogleContract({
  ...
  console.log('[googleContractGenerator] Validando template...', {
    templateId,
    rootFolderId,
    contractName,
    eventDate,
  ...
  console.log('[googleContractGenerator] Pastas OK', {
    year,
    month,
    yearFolderId,
    monthFolderId,
  ...
  console.log('[googleContractGenerator] Documento copiado', {
    docId,
