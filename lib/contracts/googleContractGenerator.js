...
async function getGoogleAuth() {
  ...
  const credentialsFromEnv = normalizeGoogleCredentials(credentialsRaw);
  let credentials = credentialsFromEnv;
  let credentialsSource = 'env';

  if (!credentialsFromEnv.refresh_token) {
    const { credentials: credentialsFromSupabase, source } = await loadGoogleCredentialsFromSupabase();
