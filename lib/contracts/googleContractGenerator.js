...
function getGoogleAuth() {
  ...
  const refreshTokenRaw = process.env.GOOGLE_OAUTH_REFRESH_TOKEN;
  const refreshToken = normalizeRefreshToken(refreshTokenRaw);

  console.info('[googleContractGenerator] OAuth env diagnostics:', {
    hasClientId: Boolean(clientId),
    hasClientSecret: Boolean(clientSecret),
    hasRedirectUri: Boolean(redirectUri),
    refreshTokenRawType: typeof refreshTokenRaw,
    refreshTokenResolved: maskToken(refreshToken),
  });
...
function normalizeRefreshToken(rawValue) {
  if (rawValue && typeof rawValue === 'object') {
    const directToken = String(rawValue.refresh_token || '').trim();
  ...
  // Aceita token simples: "1//abc..."
  const unquoted = value.replace(/^"(.*)"$/, '$1').trim();

  // Também aceita JSON serializado com refresh_token,
  // ex: {"refresh_token":"1//abc..."}
  if (unquoted.startsWith('{') && unquoted.endsWith('}')) {
    try {
      const parsed = JSON.parse(unquoted);
      if (parsed && typeof parsed === 'object') {
        const tokenFromRoot = String(parsed.refresh_token || '').trim();
        if (tokenFromRoot) return tokenFromRoot;

        const tokenFromNested = String(parsed?.tokens?.refresh_token || '').trim();
        if (tokenFromNested) return tokenFromNested;
      }
    } catch {
      // Se não for JSON válido, segue usando como token simples.
    }
  }
