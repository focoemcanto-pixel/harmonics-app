...
async function getGoogleAuth() {
  ...
  const credentialsRaw = process.env.GOOGLE_OAUTH_REFRESH_TOKEN;
  const envValidation = validateGoogleCredentials(credentialsRaw);
  let credentials = envValidation.credentials;
  let credentialsSource = 'env';
  let credentialsValidation = envValidation;

  if (!envValidation.valid) {
    const { credentials: credentialsFromSupabase, source, validation } =
  ...
  console.info('[googleContractGenerator] OAuth diagnostics:', {
    ...
    credentialsRawType: typeof credentialsRaw,
    credentialsSource,
  ...
  try {
    // Alguns runtimes/builds podem inicializar `credentials` como string,
    // o que quebra ao tentar definir `refresh_token`.
    ...
    const googleClientCredentials = buildGoogleClientCredentials(credentials);
    if (!googleClientCredentials.valid) {
      throw new Error(
        `Credenciais OAuth inválidas antes de setCredentials (${googleClientCredentials.reason}): ${googleClientCredentials.message}`
      );
    }

    oauth2Client.setCredentials(googleClientCredentials.credentials);
  } catch (error) {
