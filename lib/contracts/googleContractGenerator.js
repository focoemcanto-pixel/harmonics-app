function getGoogleAuth() {
  ...
  if (!clientId) {
    throw new Error('GOOGLE_OAUTH_CLIENT_ID não definida.');
  }

  if (!clientSecret) {
    throw new Error('GOOGLE_OAUTH_CLIENT_SECRET não definida.');
  }

  if (!redirectUri) {
    throw new Error('GOOGLE_OAUTH_REDIRECT_URI não definida.');
  }

  if (!refreshToken) {
    throw new Error('GOOGLE_OAUTH_REFRESH_TOKEN não definida.');
  }
  ...
  try {
    console.log('[googleContractGenerator] tipo do refreshToken:', typeof refreshToken);
    console.log('[googleContractGenerator] refreshToken len:', refreshToken.length);
    console.log('[googleContractGenerator] refreshToken começa com:', refreshToken.slice(0, 12));

    oauth2Client.setCredentials({
      refresh_token: refreshToken,
    });
  } catch (error) {
    console.error('[googleContractGenerator] erro ao aplicar refresh token:', error);
    throw new Error(
      `Falha ao configurar credenciais Google OAuth: ${error?.message || 'erro desconhecido'}`
    );
  }
...
export async function generateGoogleContract({
  ...
  try {
    ...
    throw new Error('Falha ao salvar o PDF do contrato no Google Drive.');
  }
