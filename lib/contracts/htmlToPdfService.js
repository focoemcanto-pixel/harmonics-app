...
function isMostlyUppercaseText(value) {
  ...
  return upperLetters.length / letters.length >= 0.82;
}

function shouldRenderHeadingAsParagraph(innerHtml) {
  const text = stripHtml(innerHtml);
  if (!text) return true;
  if (/^cl[áa]usula\s+\d+/i.test(text)) return true;
  if (text.length > 90) return true;
  if (/[.!?;:]$/.test(text)) return true;
  if (!isMostlyUppercaseText(text) && text.length > 35) return true;
  return false;
}

function normalizeContractHeadingsForPdf(value) {
  return String(value || '').replace(
    /<h([1-6])([^>]*)>([\s\S]*?)<\/h\1>/gi,
    (full, level, attrs, inner) => {
      if (!shouldRenderHeadingAsParagraph(inner)) return `<h${level}${attrs || ''}>${inner}</h${level}>`;
      return `<p>${inner}</p>`;
    }
  );
}
...
function cleanPdfHtmlArtifacts(value) {
  ...
  html = html.replace(/\n\s*\}\s*\n/g, '\n');
  html = html.replace(/(^|\n)\s*\}\s*($|\n)/g, '$1');
  html = normalizeContractHeadingsForPdf(html);
