...
function injectContractPreviewSafetyCss(value) {
  const html = String(value || '').trim();
  if (!html) return '';

  return `
<style data-harmonics-contract-preview-safety="true">
  html,
  body {
...
export function renderContractHtmlWithTemplateData(htmlInput, templateData) {
  ...
  return injectContractPreviewSafetyCss(sanitizeHtml(rendered));
}
