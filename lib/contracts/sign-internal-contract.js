...
async function resolveSigningContext({ supabase, token, contractId, precontractId }) {
  ...
  if (!contract && precontract?.id) {
    const inserted = await supabase.from('contracts').insert({ precontract_id: precontract.id, workspace_id: workspaceId, public_token: token, status: 'draft' }).select('*').single();
