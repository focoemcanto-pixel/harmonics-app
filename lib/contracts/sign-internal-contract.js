...
async function resolveFallbackWorkspaceId({ supabase }) {
  ...
  const activeWorkspace = await supabase
    .from('workspaces')
    .select('id')
    .eq('is_active', true)
...
async function resolveSigningContext({ supabase, token, contractId, precontractId }) {
  ...
  console.log('PRECONTRACT WORKSPACE', precontract?.workspace_id || null);
  console.log('EVENT WORKSPACE', event?.workspace_id || null);
  console.log('FINAL WORKSPACE', workspaceId || null);
...
export async function signInternalContract({ supabase, token, contractId, precontractId, html, signerName, signerCpf, origin, requestHeaders, ip, userAgent }) {
  ...
  try {
    ...
    const { data: persistedContract } = await supabase
      .from('contracts')
      .select('id, workspace_id')
      .eq('id', contract.id)
      .maybeSingle();
    if (!persistedContract?.workspace_id) throwWorkspaceError();
