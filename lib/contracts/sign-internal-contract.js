...
function throwWorkspaceError() {
  const e = new Error('Contrato sem workspace_id. Não foi possível concluir a geração do PDF.');
  e.status = 422;
  e.code = 'MISSING_WORKSPACE';
  throw e;
}

async function resolveFallbackWorkspaceId({ supabase }) {
  const defaultWorkspace = await supabase.from('workspaces').select('id').eq('slug', 'default').limit(1).maybeSingle();
  if (defaultWorkspace?.data?.id) return defaultWorkspace.data.id;
  const activeWorkspace = await supabase.from('workspaces').select('id').eq('is_active', true).order('created_at', { ascending: true }).limit(1).maybeSingle();
  if (activeWorkspace?.data?.id) return activeWorkspace.data.id;
  const firstWorkspace = await supabase.from('workspaces').select('id').order('created_at', { ascending: true }).limit(1).maybeSingle();
  return firstWorkspace?.data?.id || null;
}
...
function buildContractSeedFromPrecontract({ precontract, token, workspaceId }) {
  ...
  return {
    ...
    status: 'draft',
    raw_payload: { precontract_snapshot: precontract },
  };
}

async function resolveSigningContext({ supabase, token, contractId, precontractId }) {
  let contract = null; let precontract = null; let event = null;
  ...
  if (precontract?.event_id) { const { data } = await supabase.from('events').select('id, workspace_id').eq('id', precontract.event_id).maybeSingle(); event = data || null; }
  const fallbackWorkspaceId = await resolveFallbackWorkspaceId({ supabase });
  const workspaceId = precontract?.workspace_id || event?.workspace_id || fallbackWorkspaceId || null;
  ...
  if (precontractId) {
    ...
    if (!data || String(data.public_token || '') !== String(token || '')) { const e = new Error('Pré-contrato informado não pertence ao token/workspace.'); e.status = 403; e.code = 'PRECONTRACT_SCOPE_MISMATCH'; throw e; }
    precontract = data;
  }

  if (contractId) {
    const { data } = await supabase.from('contracts').select('*').eq('id', contractId).eq('workspace_id', workspaceId).maybeSingle();
    if (!data) { const e = new Error('Contrato informado não pertence ao precontract/workspace.'); e.status = 403; e.code = 'CONTRACT_SCOPE_MISMATCH'; throw e; }
    contract = data;
  }
  ...
  if (contract && String(contract.precontract_id || '') !== String(precontract.id || '')) { const e = new Error('Contrato informado não pertence ao precontract/workspace.'); e.status = 403; e.code = 'CONTRACT_SCOPE_MISMATCH'; throw e; }
  ...
  if (!contract && precontract?.id) {
    ...
    if (inserted.error) {
      const { data: existingByPrecontract } = await supabase.from('contracts').select('*').eq('precontract_id', precontract.id).eq('workspace_id', workspaceId).maybeSingle();
      if (existingByPrecontract?.id) contract = existingByPrecontract;
      else throw inserted.error;
    } else contract = inserted.data;
  }
  ...
  return { contract, precontract, event, workspaceId };
}
...
export async function signInternalContract({ supabase, token, contractId, precontractId, html, signerName, signerCpf, origin, requestHeaders, ip, userAgent }) {
  ...
  try {
    ...
    const { contract, precontract, event, workspaceId } = await resolveSigningContext({ supabase, token, contractId, precontractId });
    ...
    const finalResult = await updateContractWithFallbacks({ supabase, contractId: contract.id, patchPayload: { workspace_id: workspaceId, pdf_url: pdfUrl, signed_at: signedDocument.signedAtIso, document_hash: signedDocument.documentHash, validation_token: validationToken, verification_token: validationToken } });

    const { data: persistedContract } = await supabase.from('contracts').select('id, workspace_id, pdf_url').eq('id', contract.id).maybeSingle();
    if (!persistedContract?.workspace_id) {
      const repair = await updateContractWithFallbacks({ supabase, contractId: contract.id, patchPayload: { workspace_id: workspaceId, pdf_url: pdfUrl } });
      if (repair?.missingColumns?.includes('workspace_id')) throwWorkspaceError();
    }

    const missingColumns = [...(initial?.missingColumns||[]), ...(pdfData?.missingColumns||[]), ...(finalResult?.missingColumns||[])];
    logInfo('PUBLIC_CONTRACT_SIGN_INTERNAL_HELPER_OK', 'OK', { contractId: contract.id, precontractId: contract.precontract_id, hasHtml, hasPdfUrl: true, workspaceId });
