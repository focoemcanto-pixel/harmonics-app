...
async function resolveSigningContext({ supabase, token, contractId, precontractId }) {
  ...
  if (contractId) { const { data } = await supabase.from('contracts').select('*').eq('id', contractId).maybeSingle(); contract = data || null; }
  if (!precontract && precontractId) { const { data } = await supabase.from('precontracts').select('*').eq('id', precontractId).maybeSingle(); precontract = data || null; }
  if (!contract && token) { const { data } = await supabase.from('contracts').select('*').eq('public_token', token).maybeSingle(); contract = data || null; }
  ...
  if (!contract && precontract?.id) { const { data } = await supabase.from('contracts').select('*').eq('precontract_id', precontract.id).maybeSingle(); contract = data || null; }
  if (!contract && precontract?.id) {
    const inserted = await supabase.from('contracts').insert({ precontract_id: precontract.id, public_token: token, status: 'draft' }).select('*').single();
    if (inserted.error) throw inserted.error; contract = inserted.data;
  }
  if (!precontract && contract?.precontract_id) { const { data } = await supabase.from('precontracts').select('*').eq('id', contract.precontract_id).maybeSingle(); precontract = data || null; }
...
export async function signInternalContract({ supabase, token, contractId, precontractId, html, signerName, signerCpf, origin, requestHeaders, ip, userAgent }) {
  ...
  try {
    ...
    const { contract } = await resolveSigningContext({ supabase, token, contractId, precontractId });
    ...
    const initial = await updateContractWithFallbacks({ supabase, contractId: contract.id, patchPayload: { status: 'signed', precontract_id: contract.precontract_id, public_token: validationToken, signed_html: signedDocument.signedHtml, document_hash: signedDocument.documentHash, signed_at: signedDocument.signedAtIso, signer_ip: signerIp, user_agent: ua, validation_token: validationToken, verification_token: validationToken, signature_metadata: signatureMetadata, signature_name: signerName, raw_payload: { ...(contract.raw_payload || {}), signed_contract_html: signedDocument.signedHtml, contract_html_snapshot: signedDocument.immutableHtml, signature_metadata: signatureMetadata } } });
    const pdfData = await generateAndSaveInternalContractPdf({ supabase, contractId: contract.id, precontractId: contract.precontract_id, html: signedDocument.signedHtml });
    ...
    const finalResult = await updateContractWithFallbacks({ supabase, contractId: contract.id, patchPayload: { pdf_url: pdfUrl, signed_at: signedDocument.signedAtIso, document_hash: signedDocument.documentHash, validation_token: validationToken, verification_token: validationToken } });
