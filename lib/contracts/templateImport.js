...
function shouldKeepAsHeading(innerHtml) {
...
function normalizeContractHeadings(value) {
  return String(value || '').replace(
    /<h([1-4])([^>]*)>([\s\S]*?)<\/h\1>/gi,
    (full, level, attrs, inner) => {
      if (shouldKeepAsHeading(inner)) return `<h${level}${attrs || ''}>${inner}</h${level}>`;
      return `<p>${inner}</p>`;
    }
  );
}
...
function sanitizeHtml(value) {
  ...
  sanitized = normalizeContractHeadings(sanitized);
