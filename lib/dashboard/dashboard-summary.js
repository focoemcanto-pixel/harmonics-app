...
import { calculateFinancialSummary } from '../finance/calculateFinancialSummary';
...
function hasAdjustmentRequestInNotes(notes) {
  const normalizedNotes = String(notes || '').toUpperCase();
  return normalizedNotes.includes(ADJUSTMENT_REQUEST_MARKER);
}

function hasContractAdjustmentRequest(contract) {
  const rawAdjustment = contract?.raw_payload?.client_form?.adjustment_request;
  return !!String(rawAdjustment || '').trim();
}
...
export function buildDashboardSummary(
  ...
  eventMusicians = [],
  repertoireConfigs = [],
  contractAdjustmentRequests = [],
  options = {}
) {
  const now = new Date();
  const monthlyFinanceEvents = Array.isArray(options?.monthlyFinanceEvents)
    ? options.monthlyFinanceEvents
    : events;
  const eventosMes = monthlyFinanceEvents.filter((ev) => isSameMonth(ev.event_date, now));
  const monthlyFinancialSummary = options?.monthlyFinancialSummary
    || calculateFinancialSummary(eventosMes, []);
  ...
  const pendingAdjustmentRows = Array.isArray(contractAdjustmentRequests)
    ? contractAdjustmentRequests.filter(
        (item) => String(item?.status || '').trim().toLowerCase() === 'pending'
      )
    : [];

  const pendingAdjustmentByPrecontractId = new Map(
    pendingAdjustmentRows
      .filter((item) => item?.precontract_id)
      .map((item) => [String(item.precontract_id), item])
  );

  const adjustmentCandidates = precontracts
    .filter((precontract) => {
      if (!precontract?.id) return false;
      if (pendingAdjustmentByPrecontractId.has(String(precontract.id))) return true;

      // fallback legado (antes da tabela dedicada)
      const status = normalizeStatus(precontract?.status);
      if (status !== 'CLIENT_FILLING') return false;
      if (hasAdjustmentRequestInNotes(precontract?.notes)) return true;
      const contract = contractsByPrecontractId.get(String(precontract?.id || ''));
      return hasContractAdjustmentRequest(contract);
    })
    .map((precontract) => {
      const event = eventsById.get(String(precontract?.event_id || ''));
      const contract = contractsByPrecontractId.get(String(precontract?.id || ''));
      ...
      return {
        ...
        eventId: precontract?.event_id || event?.id || null,
        publicToken:
          precontract?.public_token ||
          contract?.public_token ||
          contract?.raw_payload?.precontract_snapshot?.public_token ||
          null,
        clientName: clientResolution.value,
        clientNameSource: clientResolution.source,
        eventDate,
  ...
  const ajustesSolicitadosPendentes = adjustmentCandidates.length;
  const ajusteSolicitadoMaisUrgente = adjustmentCandidates[0] || null;

  return {
    receitaContratada: monthlyFinancialSummary.receitaContratada,
    recebido: monthlyFinancialSummary.recebido,
    emAberto: monthlyFinancialSummary.emAberto,
    custosTotais: monthlyFinancialSummary.custosTotais,
    lucroPrevisto: monthlyFinancialSummary.lucroPrevisto,
    lucroRealizado: monthlyFinancialSummary.lucroRealizado,
    pagos: monthlyFinancialSummary.pagos,
    parciais: monthlyFinancialSummary.parciais,
    pendentes: monthlyFinancialSummary.pendentes,
    comprovantesEmValidacao: monthlyFinancialSummary.comprovantesEmValidacao,
    bruto: monthlyFinancialSummary.receitaContratada,
    liquido: monthlyFinancialSummary.lucroPrevisto,
    ...
    ajustesSolicitadosPendentes,
    ajusteSolicitadoMaisUrgente,
  };
}
