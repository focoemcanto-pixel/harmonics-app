...
export function canResendInvite(escala) {
  if (!escala?.invite_sent_at) return false;
...
/**
 * Formata status do convite para exibição
 * @param {object} escala
 * @returns {string}
 */
export function getInviteStatusLabel(escala) {
  if (!escala.musician_email) return 'Sem email';
  if (!escala.invite_sent_at) return 'Não enviado';
  if (escala.responded_at) return 'Respondido';
  return 'Enviado';
}
