import { toNumber } from './eventos-format';

export function normalizeFormation(value) {
  const s = String(value || '').trim().toLowerCase();
  if (!s) return '';
  if (s.startsWith('solo')) return 'Solo';
  if (s.startsWith('duo')) return 'Duo';
  if (s.startsWith('trio')) return 'Trio';
  if (s.startsWith('quart')) return 'Quarteto';
  if (s.startsWith('quint')) return 'Quinteto';
  if (s.startsWith('sext')) return 'Sexteto';
  if (s.startsWith('sept')) return 'Septeto';
  return value;
}

export function getDefaultPricing() {
  return {
    price_solo: 600,
    price_duo: 1100,
    price_trio: 1600,
    price_quarteto: 1900,
    price_quinteto: 2200,
    price_sexteto: 2400,
    price_septeto: 0,

    sound_price: 600,

    reception_duo_1h: 500,
    reception_duo_2h: 800,
    reception_duo_3h: 1100,

    reception_trio_1h: 800,
    reception_trio_2h: 1100,
    reception_trio_3h: 1400,

    reception_quarteto_1h: 1100,
    reception_quarteto_2h: 1400,
    reception_quarteto_3h: 1700,

    reception_quinteto_1h: 1400,
    reception_quinteto_2h: 1700,
    reception_quinteto_3h: 2000,

    reception_sexteto_1h: 1700,
    reception_sexteto_2h: 2000,
    reception_sexteto_3h: 2300,

    reception_septeto_1h: 2000,
    reception_septeto_2h: 2300,
    reception_septeto_3h: 2600,

    musician_unit_cost: 250,
    sound_default_cost: 350,
    transport_default_cost: 200,
    other_default_cost: 0,
    custom_costs: [],
    finance_notes: '',
  };
}

export function sanitizeCustomCosts(value) {
  const input = Array.isArray(value) ? value : [];
  return input
    .map((item) => ({
      label: String(item?.label || '').trim(),
      amount: toNumber(item?.amount),
    }))
    .filter((item) => item.label && item.amount >= 0);
}

export function getCustomCostsTotal(customCosts) {
  return sanitizeCustomCosts(customCosts).reduce((acc, item) => acc + toNumber(item.amount), 0);
}

export function buildCostBreakdown({
  musicianCost = 0,
  soundCost = 0,
  extraTransportCost = 0,
  customCosts = [],
} = {}) {
  const breakdown = [
    { key: 'musician', label: 'Músicos', amount: toNumber(musicianCost) },
    { key: 'sound', label: 'Som', amount: toNumber(soundCost) },
    { key: 'transport', label: 'Transporte', amount: toNumber(extraTransportCost) },
  ];

  for (const item of sanitizeCustomCosts(customCosts)) {
    breakdown.push({
      key: 'custom',
      label: item.label,
      amount: toNumber(item.amount),
    });
  }

  return breakdown;
}

function pickPricingNumber(primaryValue, fallbackValue) {
  const hasPrimary = primaryValue !== undefined && primaryValue !== null && String(primaryValue).trim() !== '';
  if (hasPrimary) return toNumber(primaryValue);
  return toNumber(fallbackValue);
}

export function buildCanonicalFormationPrices(pricing = {}) {
  const source = pricing?.formation_prices && typeof pricing.formation_prices === 'object'
    ? pricing.formation_prices
    : {};

  return {
    solo: pickPricingNumber(pricing.price_solo, source.solo),
    duo: pickPricingNumber(pricing.price_duo, source.duo),
    trio: pickPricingNumber(pricing.price_trio, source.trio),
    quarteto: pickPricingNumber(pricing.price_quarteto, source.quarteto),
    quinteto: pickPricingNumber(pricing.price_quinteto, source.quinteto),
    sexteto: pickPricingNumber(pricing.price_sexteto, source.sexteto),
    septeto: pickPricingNumber(pricing.price_septeto, source.septeto),
  };
}

export function hydratePricingFromCanonical(pricing = {}) {
  const canonical = buildCanonicalFormationPrices(pricing);
  return {
    ...pricing,
    formation_prices: canonical,
    price_solo: canonical.solo,
    price_duo: canonical.duo,
    price_trio: canonical.trio,
    price_quarteto: canonical.quarteto,
    price_quinteto: canonical.quinteto,
    price_sexteto: canonical.sexteto,
    price_septeto: canonical.septeto,
  };
}

export function getAutomaticFormationPrice(formation, pricing) {
  const f = normalizeFormation(formation);
  if (!f) return 0;
  const canonical = buildCanonicalFormationPrices(pricing);

  switch (f) {
    case 'Solo':
      return toNumber(canonical.solo);
    case 'Duo':
      return toNumber(canonical.duo);
    case 'Trio':
      return toNumber(canonical.trio);
    case 'Quarteto':
      return toNumber(canonical.quarteto);
    case 'Quinteto':
      return toNumber(canonical.quinteto);
    case 'Sexteto':
      return toNumber(canonical.sexteto);
    case 'Septeto':
      return toNumber(canonical.septeto);
    default:
      return 0;
  }
}

export function getAutomaticReceptionPrice(formation, hours, pricing) {
  const f = normalizeFormation(formation);
  const h = parseInt(hours, 10) || 0;
  if (!h || !f) return 0;

  if (f === 'Duo') return toNumber(pricing[`reception_duo_${h}h`]);
  if (f === 'Trio') return toNumber(pricing[`reception_trio_${h}h`]);
  if (f === 'Quarteto') return toNumber(pricing[`reception_quarteto_${h}h`]);
  if (f === 'Quinteto') return toNumber(pricing[`reception_quinteto_${h}h`]);
  if (f === 'Sexteto') return toNumber(pricing[`reception_sexteto_${h}h`]);
  if (f === 'Septeto') return toNumber(pricing[`reception_septeto_${h}h`]);

  return 0;
}

export function getPaymentStatus(agreed, paid) {
  const a = toNumber(agreed);
  const p = toNumber(paid);

  if (a <= 0) return 'Pendente';
  if (p <= 0) return 'Pendente';
  if (p >= a) return 'Pago';
  return 'Parcial';
}

export function getPaymentTone(paymentStatus) {
  if (paymentStatus === 'Pago') return 'emerald';
  if (paymentStatus === 'Parcial') return 'amber';
  if (paymentStatus === 'Pendente') return 'red';
  return 'default';
}


export function getFormationMusicianCount(formation) {
  const f = normalizeFormation(formation);
  if (f === 'Solo') return 1;
  if (f === 'Duo') return 2;
  if (f === 'Trio') return 3;
  if (f === 'Quarteto') return 4;
  if (f === 'Quinteto') return 5;
  if (f === 'Sexteto') return 6;
  if (f === 'Septeto') return 7;
  return 0;
}

export function getAutomaticCosts({ formation, hasSound, hasTransport, transportPrice, pricing }) {
  const musicianCount = getFormationMusicianCount(formation);
  const musicianUnitCost = toNumber(pricing?.musician_unit_cost);
  const soundDefaultCost = toNumber(pricing?.sound_default_cost);
  const transportDefaultCost = toNumber(pricing?.transport_default_cost);
  const customCostsTotal = getCustomCostsTotal(pricing?.custom_costs);
  const legacyOtherDefaultCost = toNumber(pricing?.other_default_cost);
  const otherDefaultCost = customCostsTotal > 0 ? customCostsTotal : legacyOtherDefaultCost;
  const costBreakdown = buildCostBreakdown({
    musicianCost: musicianCount * musicianUnitCost,
    soundCost: hasSound ? soundDefaultCost : 0,
    extraTransportCost: hasTransport || toNumber(transportPrice) > 0 ? transportDefaultCost : 0,
    customCosts: pricing?.custom_costs,
  });

  return {
    musicianCost: musicianCount * musicianUnitCost,
    soundCost: hasSound ? soundDefaultCost : 0,
    extraTransportCost: hasTransport || toNumber(transportPrice) > 0 ? transportDefaultCost : 0,
    otherCost: otherDefaultCost,
    costBreakdown,
  };
}
