...
export function addHoursToTime(timeValue, deltaHours = 0) {
  ...
  const [h, m] = String(timeValue).split(':').map((v) => Number(v || 0));
...
export function getEventMeta(event = {}) {
  ...
  const isDone = String(event.status || '').toLowerCase() === 'done';
...
export function buildMemberDashboardData({
  ...
  const rows = invites.map((invite) => {
    ...
    const textSources = [invite.message, event.observations].filter(Boolean).join('\n');
    const youtubeUrls = extractYoutubeUrlsFromText(textSources);
    ...
    return {
      ...
      hasSound: event.has_sound || false,
      receptionHours: event.reception_hours || 0,
      eventStatus: event.status || '',
      isDone: meta.isDone,
      weekday: getWeekdayPTBR(event.event_date),
      mapsUrl: buildMapsUrl(event.location_name),
      isDone:
        String(event.status || '').toLowerCase() === 'done' ||
  ...
  const proximosConfirmados = [...confirmados].sort((a, b) => {
    const aDate = `${a.eventDate || ''}T${a.eventTime || '00:00:00'}`;
    const bDate = `${b.eventDate || ''}T${b.eventTime || '00:00:00'}`;
