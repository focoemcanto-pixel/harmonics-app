...
export function getEventMeta(event = {}) {
  ...
  const antesalaDurationMinutes = Number(event?.antesala_duration_minutes ?? 0) || 0;
  const hasAntesala = Boolean(
    event?.has_antesala ??
      event?.antesala_enabled ??
      (antesalaDurationMinutes > 0)
  );
...
export function buildMemberDashboardData({
  ...
  const precontractByEventId = new Map(
    safePrecontracts
  ...
  const rows = safeInvites.map((invite) => {
    const eventBase = normalizeEventRelation(invite?.events);
    const precontract = precontractByEventId.get(String(invite?.event_id)) || null;
    const event = {
      ...eventBase,
      has_reception: eventBase?.has_reception ?? eventBase?.has_receptivo ?? null,
      reception_hours:
        eventBase?.reception_hours ??
        eventBase?.receptivo_hours ??
        precontract?.reception_hours ??
        0,
    };
    ...
    return {
      ...
      hasAntesala: meta.hasAntesala,
      antesalaDurationMinutes: meta.antesalaDurationMinutes,
      ...
      repertoireConfig,
      repertorioPdfUrl:
        repertoireConfig?.repertoire_pdf_url || '',
      repertorioItems,
      youtubeUrls,
  ...
  const hasActiveRepertoire = (row) => {
    const status = String(row?.repertoireConfig?.status || '').trim().toUpperCase();
  ...
  return {
    ...
    resumo: {
      ...
      repertorios: confirmados.filter((row) => hasActiveRepertoire(row)).length,
    },
  };
}
