...
export async function deletePaymentsByIds({ supabase, paymentIds = [], logPrefix = '[PAYMENTS_DELETE_API]' }) {
  ...
  const existingPaymentsQuery = supabase
    .from('payments')
    .select('id, event_id, proof_file_url');

  const { data: existingPayments, error: existingError } = normalizedIds.length === 1
    ? await existingPaymentsQuery.eq('id', normalizedIds[0])
    : await existingPaymentsQuery.in('id', normalizedIds);
  ...
  const { data: deletedRows, error: deleteError } = normalizedIds.length === 1
    ? await deleteQuery.eq('id', normalizedIds[0]).select('id, event_id')
    : await deleteQuery.in('id', normalizedIds).select('id, event_id');
