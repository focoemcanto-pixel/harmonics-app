...
function renderCustomRepertoireSection(payload = {}) {
  if (!payload?.isCustomEvent) return '';

  const selectedStyles = Array.isArray(payload?.selectedStyles)
    ? payload.selectedStyles.map((entry) => compactText(entry)).filter(Boolean)
    : [];
  const preferredArtists = compactText(payload?.preferredArtists || '');
  const customSongs = Array.isArray(payload?.customSongs) ? payload.customSongs : [];

  const hasContent =
    selectedStyles.length > 0 || hasValue(preferredArtists) || customSongs.length > 0;
  ...
  return `
    <section class="section-block">
      <div class="section-head">
        <h2>CUSTOM</h2>
        <p>Preferências personalizadas para eventos fora do fluxo tradicional de casamento.</p>
      </div>

      ${selectedStyles.length > 0 ? `
      <article class="music-card">
        <h3 class="music-card__title">🎼 Estilos musicais desejados</h3>
        <div class="chip-list">
          ${selectedStyles.map((style) => `<span class="chip">${escapeHtml(style)}</span>`).join('')}
        </div>
      </article>
      ` : ''}

      ${hasValue(preferredArtists) ? `
      <article class="music-card">
        <h3 class="music-card__title">🎤 Artistas desejados</h3>
        <p class="music-card__line">${escapeHtml(preferredArtists)}</p>
      </article>
      ` : ''}

      ${customSongs.length > 0 ? `
      <div class="moment-group">
        <h3 class="moment-group__title">Músicas específicas</h3>
        <div class="card-grid">
          ${customSongs.map((song, index) => {
            const songName = compactText(song?.songName) || '—';
            const referenceTitle = compactText(song?.referenceTitle || '');
            const referenceLink = compactText(song?.referenceLink || '');
            const artists = compactText(song?.artists || '');
            const notes = compactText(song?.notes || '');

            return `
              <article class="music-card">
                <div class="music-card__badge">Música ${index + 1}</div>
                <p class="music-card__line"><strong>🎵 Música:</strong> ${escapeHtml(songName)}</p>
                ${hasValue(referenceLink)
                  ? `<p class="music-card__line music-card__line--secondary"><strong>🔗 Referência:</strong> <a href="${escapeHtml(referenceLink)}" target="_blank" rel="noreferrer">${escapeHtml(referenceTitle || referenceLink)}</a></p>`
                  : ''}
                ${hasValue(artists)
                  ? `<p class="music-card__line"><strong>👤 Artista/canal:</strong> ${escapeHtml(artists)}</p>`
                  : ''}
                ${hasValue(notes)
                  ? `<p class="music-card__line music-card__line--notes"><strong>📝 Observação:</strong> ${escapeHtml(notes)}</p>`
                  : ''}
              </article>
            `;
          }).join('')}
...
function mapSectionItems(items = [], sectionKey, badgeLabel) {
  return items
    .filter((item) => normalizeSectionKey(item?.section) === sectionKey)
    .map((item, index) => ({
...
function mapSectionGroup(items = [], sectionKey, badgeLabel) {
  ...
  sectionItems.forEach((item) => {
    const normalizedMoment = sanitizeSingleLine(item?.moment);
    const card = {
      badge: badgeLabel(sectionIndex),
      title: normalizeItemTitle(item),
      songName: item?.song_name,
      referenceLink: item?.reference_link,
      notes: item?.notes,
    ...
    const momentKey = normalizedMoment.toLowerCase();
    if (!moments.has(momentKey)) {
      moments.set(momentKey, {
        title: normalizedMoment.toUpperCase(),
        cards: [],
      });
    }
    moments.get(momentKey).cards.push(card);
  });

  return {
    root,
    moments: Array.from(moments.values()),
  };
}

function buildSection({ title, subtitle, cards, momentGroups = [] }) {
  const safeCards = Array.isArray(cards) ? cards : [];
  const safeMomentGroups = Array.isArray(momentGroups) ? momentGroups : [];
  const hasContent = safeCards.length > 0 || safeMomentGroups.some((group) => (group?.cards || []).length > 0);
  if (!hasContent) return '';
  ...
  return `
    <section class="section-block">
      ...
      ${safeCards.length > 0 ? `
      <div class="card-grid">
        ${safeCards.map((card) => buildCard(card)).join('')}
      </div>
      ` : ''}
      ${safeMomentGroups
        .filter((group) => Array.isArray(group?.cards) && group.cards.length > 0)
        .map((group) => `
          <div class="moment-group">
            <h3 class="moment-group__title">${escapeHtml(group.title)}</h3>
            <div class="card-grid">
              ${group.cards.map((card) => buildCard(card)).join('')}
            </div>
          </div>
        `)
        .join('')}
...
export function buildRepertoirePdfHtml({
  event = {},
  config = {},
  items = [],
  eventTypeLabel = '',
  ...
  const normalizedEventTypeLabel = compactText(eventTypeLabel);
  const pdfTitle = normalizedEventTypeLabel
    ? `REPERTÓRIO — ${normalizedEventTypeLabel.toUpperCase()}`
    : 'REPERTÓRIO — CERIMONIAL MUSICAL';
  ...
  const cerimoniaGroups = mapSectionGroup(sortedItems, 'cerimonia', () => 'Momento');
  ...
  const sectionsHtml = [
    ...
    buildSection({
      title: 'CERIMÔNIA',
      subtitle: 'Momentos musicais durante o rito principal.',
      cards: cerimoniaGroups.root,
      momentGroups: cerimoniaGroups.moments,
    }),
...
<html lang="pt-BR">
  ...
  <body>
    <main class="page cover">
      ...
      <h2 class="cover-title">🎵 ${escapeHtml(pdfTitle)}</h2>
