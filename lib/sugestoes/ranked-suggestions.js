import { logSuggestionScope, stripCrossWorkspaceRelations } from '@/lib/sugestoes/workspace-scope';
...
const RANKED_SELECT_FIELDS = `
  ...
  source_type,
  created_at,
  genre:suggestion_genres(id, workspace_id, name, slug),
  moment:suggestion_moments(id, workspace_id, name, slug)
`;
...
export async function getRankedSuggestions(supabase, options = {}) {
  ...
  if (!workspaceId) {
    logSuggestionScope('[sugestoes] ranked suggestions blocked without workspaceId');
    return [];
  }
  ...
  logSuggestionScope('[sugestoes] ranked suggestions result', { workspaceId, count: (data || []).length });

  return (data || [])
    .map((song) => stripCrossWorkspaceRelations(song, workspaceId))
    .map((song) => ({
      ...song,
