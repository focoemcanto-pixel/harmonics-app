const INSTRUMENT_EQUIVALENCE_RULES = [
  ...
  },
  {
    ...
    aliases: ['voz masculina', 'vocal masculino', 'cantor masculino', 'voz homem'],
  },
  {
    canonical: 'voz feminina',
    aliases: ['voz feminina', 'vocal feminino', 'cantora feminina', 'voz mulher'],
  },
  {
    canonical: 'voz',
...
function canonicalizeInstrumentText(value) {
  let text = stripAccentsLower(value);
  if (!text) return '';

  text = text
    .replace(/voz masculina\s+e\s+feminina/g, 'voz masculina, voz feminina')
    .replace(/vocal masculino\s+e\s+feminino/g, 'voz masculina, voz feminina')
    .replace(/voz feminina\s+e\s+masculina/g, 'voz feminina, voz masculina')
    .replace(/vocal feminino\s+e\s+masculino/g, 'voz feminina, voz masculina')
  ...
  for (const rule of INSTRUMENT_EQUIVALENCE_RULES) {
    for (const alias of rule.aliases) {
      ...
      text = text.replace(new RegExp(`\\b${escapedAlias}\\b`, 'g'), rule.canonical);
    }
  }
...
export function normalizeText(value) {
  return canonicalizeInstrumentText(value);
}

function normalizeInstrumentToken(value) {
  const normalized = canonicalizeInstrumentText(value);
  ...
  for (const rule of INSTRUMENT_EQUIVALENCE_RULES) {
    ...
    if (normalized === rule.canonical || normalized.includes(rule.canonical)) {
      return rule.canonical;
    }
  }
...
export function pickBestScaleTemplate(
  ...
  const formationNormalized = stripAccentsLower(eventFormation);

  const candidates = (templates || []).filter(
    (template) =>
      template.is_active !== false &&
      stripAccentsLower(template.formation) === formationNormalized
  );
