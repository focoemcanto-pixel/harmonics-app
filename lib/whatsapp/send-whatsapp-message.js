...
export async function sendWhatsAppMessage({ to, message, channel: channelOverride = null }) {
  ...
  if (isChannelValid(channelOverride)) {
    ...
    provider = channelOverride.provider || 'wasender';
  ...
  try {
    if (channelSource !== 'database') {
      ...
      if (isChannelValid(channel)) {
        ...
        provider = channel.provider || 'wasender';
        channelSource = 'database';
      }
    }
  ...
  const resolvedUrl =
    String(provider || 'wasender').toLowerCase() === 'wasender'
      ? resolveWaSenderSendEndpoint(apiUrl)
      : apiUrl;
  ...
  // Compatibilidade com integrações legadas que dependem de instanceId
  if (instanceId) {
