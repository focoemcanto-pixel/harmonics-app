...
const DEFAULT_WORKSPACE_SLUG = process.env.DEFAULT_WORKSPACE_SLUG || null;
...
function isLegacyProfileAdmin(profile) {
  ...
  return profile?.is_admin === true || role === 'owner' || role === 'admin' || role === 'administrador';
}
...
function isDefaultWorkspace(workspace) {
  ...
  return (
    (DEFAULT_WORKSPACE_ID && id === DEFAULT_WORKSPACE_ID) ||
    slug === DEFAULT_WORKSPACE_SLUG ||
    slug === DEFAULT_WORKSPACE_KEY ||
...
async function fetchDefaultWorkspace(client) {
  const attempts = [
    { select: WORKSPACE_SELECT, column: 'id', value: DEFAULT_WORKSPACE_ID },
...
async function buildWorkspaceContext({ supabase, workspaceId, member = null, profile = null, source }) {
  ...
  if (!workspace?.id || workspace.status !== 'active') return null;
...
export async function getCurrentWorkspace({ supabase, request } = {}) {
  ...
  if (userId && requestedWorkspaceId) {
    ...
    if (member?.workspace_id) {
      const requestedContext = await buildWorkspaceContext({
        ...
        member,
        profile,
        source: 'request_workspace_member',
      });

      if (requestedContext) return requestedContext;
    }

    if (isLegacyProfileAdmin(profile)) {
      const requestedWorkspace = await fetchWorkspaceById({
        supabase: client,
        workspaceId: requestedWorkspaceId,
      }).catch(() => null);

      if (isDefaultWorkspace(requestedWorkspace)) {
        const requestedContext = await buildWorkspaceContext({
          supabase: client,
          workspaceId: requestedWorkspace.id,
          member: null,
          profile,
          source: 'request_default_workspace_profile_admin',
        });

        if (requestedContext) return requestedContext;
      }

      console.warn('[WORKSPACE_CONTEXT][IGNORED_REQUEST_WORKSPACE]', {
        workspaceId: requestedWorkspaceId,
        reason: 'legacy_admin_without_membership',
      });
    }
  }
  ...
  if (userId) {
    ...
    if (member?.workspace_id) {
      ...
      if (memberContext) {
        console.log({
          ...
          resolvedWorkspaceId: memberContext.workspaceId,
          membershipWorkspaceId: member.workspace_id,
          authUserId: userId,
          fallbackReason: null,
        });
    ...
    if (isLegacyProfileAdmin(profile)) {
      const defaultWorkspace = await getDefaultWorkspace({ supabase: client }).catch((error) => {
        console.warn('[WORKSPACE_CONTEXT][DEFAULT_FOR_PROFILE_ADMIN]', {
          message: error?.message,
        });
        return null;
      });
      ...
      if (defaultWorkspace?.id) {
        const defaultMember = await getMembershipForWorkspace({
          supabase: client,
          userId,
          workspaceId: defaultWorkspace.id,
        });

        const defaultContext = await buildWorkspaceContext({
          supabase: client,
          workspaceId: defaultWorkspace.id,
          member: defaultMember,
          profile,
          source: defaultMember?.workspace_id ? 'default_workspace_member' : 'default_workspace_profile_admin',
        });
  ...
  const context = {
    ...
    member: null,
    source: userId ? 'default_workspace_env' : 'default_workspace_no_auth_fallback',
  };
  ...
  console.log({
    ...
    authUserId: userId,
    fallbackReason: userId ? 'authenticated_without_active_membership' : 'no_authenticated_user',
  });
